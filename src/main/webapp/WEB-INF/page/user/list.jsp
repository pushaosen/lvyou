	...
	<div class="panel-body padding-small" id="woUserSearchPanelBody">
		<form onsubmit="return false;" class="form-inline"
			...
			<button type="button" class="button button-small bg-main"
				onclick="Wobb.searchUser();"  onfocus="$('#woUserPageStart').val(0)">
...
<div class="panel" id="woUserListPanel">
	<div class="panel-head" id="woUserListPanelHeader">
		<!-- woUserSearchPageForm -->
		<form onsubmit="return false;" id="woUserSearchPageForm">
			<span class='icon-search float-right' id="woUserListPanelShowSearch"
				onclick="$('#woUserSearchPanel').removeClass ('hidden')"></span>
			<!-- <h4>用户列表</h4> -->
			<span> <span class='icon-step-backward'
				onclick="Wobb.loadUserFirstPage();"> </span> <span
				class='icon-backward' onclick="Wobb.loadUserLastPage();"></span> <span
				class="text-gray" id="woUserPageText" style="color: black;">用户列表</span>
				<span class='icon-forward' onclick="Wobb.loadUserNextPage();">
			</span> <span class='icon-step-forward' onclick="Wobb.loadUserFinalPage();">
			</span> <span class='icon-refresh' onclick="Wobb.searchUser();"></span>
			</span> <input type="hidden" class="input" id="woUserPageStart"
				name="woPageStart" value="0" /> <input type="hidden" class="input"
				id="woUserPageSize" name="woPageSize" value="10" />
		</form>
	</div>
		...
		<form method="post" id="woUserListPanelHeaderForm">
			<div class="padding-small border-bottom">
				...
				<button type="button"
					...
					onclick="Wobb.loadUserCreateForm();">
					<span>创建</span>
				</button>
				<button type="button"
					class="button button-small text-white bg-main icon-plus-circle"
					onclick="Wobb.loadUserUpdateForm();">
					<span>修改</span>
				</button>
				<button type="button"
					class="button button-small text-white bg-yellow icon-minus-circle"
					onclick="Wobb.deleteUsers ();">
					<span>删除</span>
				</button>
				<button type="button"
					class="button button-small text-white bg-yellow icon-minus-circle"
					onclick="Wobb.loadUserImportForm ();">
					<span>导入</span>
				</button>
				<button type="button"
					class="button button-small text-white bg-yellow icon-minus-circle"
					onclick="Wobb.exportUsers();">
					<span>导出</span>
				</button>
			</div>
